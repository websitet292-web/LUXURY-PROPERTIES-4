...
async function renderAdminTaskRange() {
  ...
  taskSettingUsers = usersRes.users || [];
  ...
  return `
    <div class="min-h-screen bg-[#07090d] flex">
      ...
      <div
        id="mobile-sidebar"
        class="sidebar-drawer lg:hidden ${state.sidebarOpen ? 'open' : ''}"
      >
        ${renderAdminSidebar('#/admin/task-range')}
      </div>
      ...
      <main class="flex-1 p-5 md:p-8 max-w-4xl mx-auto overflow-y-auto">
        ...
        <header class="pb-6 border-b border-[#1f2636]/60 mb-6">
          <h2 class="text-xl font-bold text-white">
            User Task & Reward Settings 👤
          </h2>

          <p class="text-xs text-slate-400">
            Search a user and set their individual task limit and reward.
          </p>
        </header>
        ...
        <div class="luxury-card p-6 max-w-lg">
          ...
          <div class="mb-5">
            ...
            <p class="text-[11px] text-slate-400 mt-1">
              Set a custom task limit and reward for an individual user.
            </p>
          </div>
...
async function loadSelectedUserTaskLimit() {
  ...
  const limitInput = document.getElementById('user-task-limit');
  const rewardInput = document.getElementById('user-task-reward');

  if (!select || !limitInput || !rewardInput) return;
...
async function saveUserTaskLimit() {
  ...
  let taskReward = parseFloat(rewardInput.value);
  ...
  try {
    ...
    if (message) {
      ...
      message.textContent =
        `Saved successfully: ${taskLimit} tasks × LKR ${taskReward.toFixed(2)} per task`;
